...
def copy_with_verification(src: str, dest: str):
    """
    Copy a file and verify that the copy has been completed successfully.
    """
    shutil.copy2(src, dest)  # Copy file with metadata
    src_size = os.path.getsize(src)
    dest_size = os.path.getsize(dest)

    # Wait until the destination file size matches the source file size
    ...
    while dest_size != src_size:
        ...
        if time.time() - start_time > 5:  # 5-second timeout
            raise RuntimeError(f"File copy verification failed for {src} -> {dest}")
...
async def predict(
    ...
    try:
        ...
        if encrypted_data and evaluation_key:
            ...
            try:
                # 파일 복사
                copy_with_verification(server_zip_path, os.path.join(tmpdirname, os.path.basename(server_zip_path)))
                copy_with_verification(client_zip_path, os.path.join(tmpdirname, os.path.basename(client_zip_path)))
